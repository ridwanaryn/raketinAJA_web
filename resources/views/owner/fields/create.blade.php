...
<div class="max-w-screen-md mx-auto px-6 py-12">
    ...
    <div class="mb-8">
        ...
        <h1 class="font-headline font-black text-4xl text-on-surface">Add New Court</h1>
        <p class="text-on-surface-variant font-medium mt-1">Register a new performance sports venue to the raketinAJA registry.</p>
    </div>

    <!-- Card Form -->
    <div class="bg-surface-container-lowest p-8 rounded-lg shadow-sm border border-surface-variant/20">
        <form action="{{ route('owner.fields.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="flex flex-col space-y-1">
                ...
                <input class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all placeholder:text-on-surface-variant/40" id="name" name="name" value="{{ old('name') }}" placeholder="Apex Padel Arena B" type="text" required/>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col space-y-1">
                    ...
                    <select class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all" id="sport_type" name="sport_type" required>
                        <option value="padel" {{ old('sport_type') === 'padel' ? 'selected' : '' }}>Padel</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col space-y-1">
                    ...
                    <input class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all placeholder:text-on-surface-variant/40" id="capacity" name="capacity" value="{{ old('capacity', 4) }}" placeholder="4" type="number" min="1" required/>
                </div>
                ...
                <div class="flex flex-col space-y-1">
                    ...
                    <select class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all" id="is_indoor" name="is_indoor" required>
                        <option value="1" {{ old('is_indoor') === '1' ? 'selected' : '' }}>Indoor</option>
                        <option value="0" {{ old('is_indoor') === '0' ? 'selected' : '' }}>Outdoor</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex flex-col space-y-1">
                ...
                <input class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all placeholder:text-on-surface-variant/40" id="location" name="location" value="{{ old('location') }}" placeholder="Chamartín, Madrid" type="text" required/>
            </div>
            ...
            <div class="flex flex-col space-y-1">
                ...
                <input class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all placeholder:text-on-surface-variant/40" id="features" name="features" value="{{ old('features') }}" placeholder="Pro-Turf, Showers, Locker Rooms, Night Lights" type="text"/>
            </div>
            ...
            <div class="flex flex-col space-y-1">
                ...
                <input class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all placeholder:text-on-surface-variant/40" id="image_url" name="image_url" value="{{ old('image_url') }}" placeholder="https://images.unsplash.com/..." type="url"/>
            </div>

            <!-- Status -->
            <div class="flex flex-col space-y-1">
                <label class="text-xs font-headline font-bold uppercase tracking-widest text-secondary px-2" for="status">Initial Status</label>
                <select class="w-full px-5 py-3 rounded-xl bg-surface-variant border-none focus:ring-2 focus:ring-primary focus:bg-surface-container-lowest transition-all" id="status" name="status" required>
                    ...
                    <option value="maintenance" {{ old('status') === 'maintenance' ? 'selected' : '' }}>Maintenance Needed</option>
                    <option value="cleaning" {{ old('status') === 'cleaning' ? 'selected' : '' }}>Cleaning Pending</option>
                </select>
            </div>
